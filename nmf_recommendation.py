...
def nmf_recommendation(user_id):
    ...
    gs = GridSearchCV(NMF, param_grid=param_grid, measures=['rmse', 'mae'], cv=5)
    gs.fit(data)
    ...
    accuracies = []
    kf = KFold(n_splits=5)
    algo = NMF(n_epochs=60, n_factors=50, random_state=0)
    ...
    algo = NMF(n_epochs=20, n_factors=50, random_state=0)
    algo.fit(trainset)
